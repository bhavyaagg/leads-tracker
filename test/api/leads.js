...
it("GET all should fetch all Leads", (done) => {
  api.get('/').end((e, r) => {
    ...
    r.body.data[1].name.should.equal('Bhavya Aggarwal')
    r.body.data[1].email.should.equal('bhavya.spv@gmail.com')
    r.body.data[1].contact.should.equal('9899175630')
    done()
  })
})
...
it("PUT /2 should fetch error", (done) => {
  api.put('/2').send({
    ...
    r.statusCode.should.equal(404)
    r.body.success.should.equal(false)
    r.body.data.error.message.should.be.a('string')
    r.body.data.error.message.should.equal('Could not update the lead with id 2 (Lead not found).')
    done()
  })
})
...
it("DELETE /2 should fetch error", (done) => {
  api.delete('/2').end((e, r) => {
    ...
    r.body.success.should.equal(false)
    r.body.data.error.message.should.be.a('string')
    r.body.data.error.message.should.equal('Could not delete the lead with id 2 (Lead not found).')
    done()
  })
})
